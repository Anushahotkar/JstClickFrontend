...
const ProductVendorsList = () => {
  ...
  useEffect(() => {
    const loadVendors = async () => {
      try {
        ...
        const vendorList = Array.isArray(response) ? response : [response];
  ...
  useEffect(() => {
    ...
    const filtered = vendors.filter(
      (v) =>
        v.shopName?.toLowerCase().includes(lowerQuery) ||
        v.productType?.toLowerCase().includes(lowerQuery) ||
        v.action?.toLowerCase().includes(lowerQuery)
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 sm:p-8 font-sans text-gray-800">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 sm:mb-8">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search vendor, product, or action..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="pl-10 pr-3 py-2 border border-gray-300 rounded-lg w-full focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm"
          />
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="py-3 px-6">
                      <div className={`flex items-center gap-2 p-1 rounded`}>
                        ...
                        <select
                          ...
                          className={`p-1 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-blue-500 ${getActionBgColor(
                            vendor.action
                          )}`}
                        >
                    ...
                    <td className="py-3 px-6">
                      <input
                        ...
                        value={vendor.reason || ""}
                        onChange={(e) =>
                          handleChange(vendor.productId, "reason", e.target.value)
                        }
                        placeholder="Reason"
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-2 p-1 rounded">
                      ...
                      <select
                        ...
                        className={`flex-1 p-1 border border-gray-300 rounded-md focus:ring-1 focus:ring-blue-500 text-sm ${getActionBgColor(
                          vendor.action
                        )}`}
                      >
                        <option value="Approved">Approved</option>
